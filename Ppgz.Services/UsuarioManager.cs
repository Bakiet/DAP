...
namespace Ppgz.Services
{
	public class UsuarioManager
	{
		...
		public List<aspnetusers> FindByCuentaId(int id)
		{
			return _db.aspnetusers.Where(u => u.cuentas1.Any(c => c.Id == id)).ToList();
		}

		public List<aspnetusers> FindAllNazan()
		{
		...
		/// </summary>
		public aspnetusers CrearProveedor(string userName, string nombre, string apellido, string email,
			string telefono, string cargo, bool activo, int perfilId, string password, int cuentaId)
		{
			...
			cuenta.aspnetusers1.Add(usuario);
			_db.SaveChanges();
		...
		/// </summary>
		public aspnetusers CrearMaestroProveedor(string userName, string nombre, string apellido,
			string email, string telefono, string cargo, bool activo, string password, int cuentaId)
		{
			...
			if (cuenta.aspnetusers1.SingleOrDefault(u => u.Tipo == Tipo.MaestroProveedor) != null)
			{
		...
		/// </summary>
		public aspnetusers Actualizar(string id, string nombre = null, string apellido = null, string email = null,
		...
		public void ActualizarPerfil(string usuarioId, int perfilId)
		{
			...
			{
				var cuenta = _db.cuentas.FirstOrDefault(c => c.aspnetusers1.Any(u => u.Id == usuario.Id));
				if (cuenta == null)
				{
					//TODO MERJORAR
					//ERROR DE APLICACION 
		...
		public void Eliminar(string usuarioId, int cuentaId)
		{
			...
			cuenta.aspnetusers1.Remove(usuario);
			_db.SaveChanges();
			QuitarRolesDeUsuario(usuarioId);
