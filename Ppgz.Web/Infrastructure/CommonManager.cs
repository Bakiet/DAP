...
namespace Ppgz.Web.Infrastructure
{
	...
	public class CommonManager
	{
        ...
        private static List<configuraciones> _configuraciones;
        public static List<configuraciones> GetConfiguraciones()
        {
		...
		public cuentas GetCuentaUsuarioAutenticado()
		{
			var usuarioAutenticado = GetUsuarioAutenticado();
			// TODO PASAR A STORE O RECURSO
			return _db.Database.SqlQuery<cuentas>(@"SELECT * FROM cuentas WHERE   id IN (SELECT CuentaId FROM cuentasusuarios WHERE UsuarioId = {0})", usuarioAutenticado.Id).FirstOrDefault();

		}

        public aspnetusers GetUsuarioAutenticado()
		{
