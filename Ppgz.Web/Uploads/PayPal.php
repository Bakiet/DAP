 ...
 * @version    $Id: PayPhone.php 9747 2012-07-26 02:08:08Z john $
 ...
 * @license    http://www.socialengine.com/license/
 */
class Payment_Plugin_Gateway_PayPhone extends Engine_Payment_Plugin_Abstract
{
   ...
   * Get the service API
   *
   * @return Engine_Service_PayPhone
   */
  ...
  public function getGateway()
  {
    if( null === $this->_gateway ) {
      $class = 'Engine_Payment_Gateway_PayPhone';
      ...
      array $params = array())
  {
    ...
    if( function_exists('iconv') && strlen($desc) != iconv_strlen($desc) ) {
      // PayPhone requires that DESC be single-byte characters
      $desc = @iconv("UTF-8", "ISO-8859-1//TRANSLIT", $desc);
    }

    // This is a one-time fee
    if( $package->isOneTime() ) {
      $params['driverSpecificParams']['PayPhone'] = array(
        'AMT' => $package->price,
        'DESC' => $desc,
      ...
      if( !empty($params['return_url']) ) {
        $params['driverSpecificParams']['PayPhone']['GIROPAYSUCCESSURL'] = $params['return_url']
          . ( false === strpos($params['return_url'], '?') ? '?' : '&' ) . 'giropay=1';
        $params['driverSpecificParams']['PayPhone']['BANKTXNPENDINGURL'] = $params['return_url']
          . ( false === strpos($params['return_url'], '?') ? '?' : '&' ) . 'giropay=1';
      }
      if( !empty($params['cancel_url']) ) {
        $params['driverSpecificParams']['PayPhone']['GIROPAYCANCELURL'] = $params['cancel_url']
          . ( false === strpos($params['return_url'], '?') ? '?' : '&' ) . 'giropay=1';
      }
    }
  ...
  public function getOrderDetailLink($orderId)
  {
    ...
    if( $this->getGateway()->getTestMode() ) {
      ...
      return 'https://pay.payphonetodoesposible.com/api/button/Prepare/?id=' . $orderId;
    }
  }
  ...
  public function getTransactionDetailLink($transactionId)
  {
    ...
    if( $this->getGateway()->getTestMode() ) {
      ...
      return 'https://pay.payphonetodoesposible.com/api/button/Prepare/?id=' . $transactionId;
    }
  }
  ...
  {
    return new Payment_Form_Admin_Gateway_PayPhone();
  }
