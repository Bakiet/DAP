			...
			try
			{
				...
				if (citaId1 != null)
				{
					...
					var horarioAnterior = string.Format("NUEVO: Anden {0} Riel {1} Horario {2} - {3}", 
						horarioRiel2.rieles.andenes.Anden,
						horarioRiel2.rieles.Riel,
						horarioRiel2.horarios.HoraDesde,
						horarioRiel2.horarios.HoraHasta);

					var horarioNuevo = string.Format("ANTERIOR: Anden {0} Riel {1} Horario {2} - {3}", 
						horarioRiel1.rieles.andenes.Anden,
						horarioRiel1.rieles.Riel,
						horarioRiel1.horarios.HoraDesde,
						horarioRiel1.horarios.HoraHasta);

					var correos = cita.proveedores.cuentas.aspnetusers1.Select(u => u.Email).ToArray();
				...
				return RedirectToAction("Enroque", new { fecha = horarioRiel1.Fecha.ToString("dd/MM/yyyy"), Area = "Dap"  });
			}
		...
		public void PenalizacionesExportar(string fechaDesde, string fechaHasta)
		{
			...
			foreach (var cita in citas)
			{
				dt.Rows.Add(
					...
					cita.RielesOcupados,
					cita.CantidadTotal,
					cita.proveedores.Rfc,
					cita.proveedores.Nombre1 + " " + cita.proveedores.Nombre2+ " " + cita.proveedores.Nombre3+ " " + cita.proveedores.Nombre4,
					cita.proveedores.cuentas.EsEspecial ? "SI" : "NO",
					cita.estatuscitas.Nombre);
			}
