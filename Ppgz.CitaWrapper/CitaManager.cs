    ...
    public static class CitaManager
    {
        ...
        public static bool ValidarCita(PreCita precita)
        {
          ...
          //  var ordenes = sapOrdenCompraManager.GetActivasConDetalle(proveedor.NumeroProveedor, organizacionCompras).ToList();
            ...
            var asnFuturos = db.asn.Where(asnf => asnf.citas.FechaCita > DateTime.Today && asnf.citas.ProveedorId == precita.ProveedorId).ToList();
            ...
            {
                cantidadDelDiaEnAlmacen = citasDelDiaEnAlpmacen.Sum(c => c.asn.Sum(asn => asn.Cantidad));
            }
            ...
            {
                cantidadDeLaSemanaEnAlmacen = citasDeLaSemanaEnAlmacen.Sum(c => c.asn.Sum(asn => asn.Cantidad));
            }
        ...
        public static void CancelarCita(int citaId)
        {
            ...
            cita.asn.ToList().ForEach(asn => db.asn.Remove(asn));
